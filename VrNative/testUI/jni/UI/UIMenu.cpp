...
{
	LOG( "Open" );
	app.GetGuiSys().OpenMenu( &app, app.GetGazeCursor(), MenuName.ToCStr() );
	MenuOpen = true;
}
...
{
	LOG( "Close" );
	app.GetGuiSys().CloseMenu( &app, Menu, false );
	MenuOpen = false;
}
...
void UIMenu::Create( const char *menuName )
{
	...
	Menu->Init( app.GetVRMenuMgr(), app.GetDefaultFont(), 0.0f, VRMenuFlags_t() );
