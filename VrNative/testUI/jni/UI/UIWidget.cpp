...
{
	return app.GetVRMenuMgr().ToObject( GetHandle() );
}
...
void UIWidget::AddToMenuWithParms( UIMenu *menu, UIWidget *parent, VRMenuObjectParms &parms )
{
    ...
    Menu->GetVRMenu()->AddItems( app.GetVRMenuMgr(), app.GetDefaultFont(), parmArray, parentHandle, false );
    parmArray.Clear();

    if ( parent == NULL )
    {
    	Handle = Menu->GetVRMenu()->HandleForId( app.GetVRMenuMgr(), Id );
    }
    else
    {
    	Handle = parent->GetMenuObject()->ChildHandleForId( app.GetVRMenuMgr(), Id );
    }
}
